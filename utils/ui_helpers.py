...
# ==============================================================================
# FUNGSI 4: KALKULASI RISK TO REWARD RATIO (R:R) & POTENTIAL GAIN TP2
# ==============================================================================
def calculate_rr_ratios(row):
    # Ambil Buy Range Terbawah (Range Buy Min) sebagai titik entry utama
    buy_val = _clean_num(
        row.get("Range Buy Min", row.get("Buy Min", row.get("Range Buy Max", None)))
    )
    ...
    if buy_val and sl_val and (buy_val > sl_val):
        ...
        # Hitung R:R Target 1 dari Buy Range Terbawah ke TP 1
        if tp1_val and tp1_val > buy_val:
            reward_tp1 = tp1_val - buy_val
            rr_val = round(reward_tp1 / risk, 1) if risk > 0 else 0.0
            rr_tp1_str = f"1 : {rr_val}"
            
        # Hitung Persentase Potential Gain Target 2 dari Buy Range Terbawah
        if tp2_val and tp2_val > buy_val:
            gain_tp2 = ((tp2_val - buy_val) / buy_val) * 100
            pot_gain_tp2_str = f"+{gain_tp2:.1f}%"

    return rr_tp1_str, pot_gain_tp2_str
        ...
        try:
            ...
            if df_plan is not None and not df_plan.empty:
                ...
                for idx, row in df_plan.iterrows():
                    ...
                    # Kalkulasi R:R TP1 dan Persentase Gain TP2
                    rr_tp1_val, pot_gain_tp2_val = calculate_rr_ratios(row)
                    ...
                    <div style="background: linear-gradient(135deg, #161B22 0%, #0D1117 100%); border: 1px solid #30363D; border-left: 5px solid #00E676; border-radius: 12px; padding: 18px; margin-bottom: 16px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 14px; text-align: center;">
                            ...
                            <div style="background: rgba(14, 17, 23, 0.9); padding: 12px; border-radius: 8px; border: 1px solid rgba(0, 230, 118, 0.2);">
                                <div style="font-size: 10px; color: #00E676; font-weight: 800;">Target 2 ({pot_gain_tp2_val})</div>
                    ...
                    with st.expander(
                        ...
                        with c1:
                            st.metric(
                                label="R:R ( Target 1 dari Buy Terbawah )", value=rr_tp1_val
                            )
                        with c2:
                            st.metric(
                                label="Potential Gain ( Target 2 )", value=pot_gain_tp2_val
                            )
            else:
