...
def render_tab_rsi():
    ...
    with col_left:
        ...
        # HANYA TAMPILKAN KONTAINER TABEL JIKA SUDAH ADA DATA / SESUDAH SCREENING
        ...
        if has_results:
            with st.container(border=True):
                ...
                with col_bull_btn:
                    ...
                    if st.button("🟢 Bullish", key="btn_switch_bull", use_container_width=True, type=btn_type_bull):
                        st.session_state["active_rsi_type"] = "bullish"
                        st.rerun()

                with col_bear_btn:
                    btn_type_bear = "primary" if st.session_state["active_rsi_type"] == "bearish" else "secondary"
                    if st.button("🔴 Bearish", key="btn_switch_bear", use_container_width=True, type=btn_type_bear):
                        st.session_state["active_rsi_type"] = "bearish"
                        st.rerun()

                st.markdown("<div style='margin-bottom: 4px;'></div>", unsafe_allow_html=True)

                # Tampilkan Tabel Sesuai Pilihan Button
