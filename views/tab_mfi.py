...
def _mfi_card_html(row, is_selected):
    ...
    rows += info_row(
        f"T1 {_rp(row.get('Harga Kiri'))} (MFI {_num(row.get('MFI Kiri')):.0f}) → "
        f"T2 {_rp(row.get('Harga Kanan'))} (MFI {_num(row.get('MFI Kanan')):.0f})"
    )
...
def render_tab_mfi():
    ...
    st.markdown(
        ...
        div[data-testid="stColumn"]:has(div[key="btn_run_mfi_screener"]) button {
        ...
        div[data-testid="stColumn"]:has(div[key="btn_run_mfi_screener"]) button:hover {
        ...
        /* Tombol Stop */
        div[data-testid="stColumn"]:has(div[key="btn_stop_mfi_screener"]) button {
        ...
        div[data-testid="stColumn"]:has(div[key="btn_stop_mfi_screener"]) button:hover {
            transform: translateY(-2px) scale(1.02) !important;
    ...
    # Inisialisasi State
    if "stop_mfi_scan" not in st.session_state:
        st.session_state["stop_mfi_scan"] = False

    if "active_mfi_type" not in st.session_state:
        st.session_state["active_mfi_type"] = "Bullish"

    if "selected_mfi_ticker" not in st.session_state:
        st.session_state["selected_mfi_ticker"] = None

    # ---------------------------------------------------------
    # LAYOUT UTAMA: SPLIT SCREEN (KIRI 38% : KANAN 62%)
    # ---------------------------------------------------------
    with keyed_container("zworkspace_mfi"):
    ...
    with col_left:
        st.markdown(
            ...
            <div class="cyber-header-container">
                ...
                <div style="display: flex; justify-content: center; align-items: center; gap: 8px;">
                    ...
                    <span class="cyber-header-title">MFI REVERSAL</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with col_btn_stop:
            stop_clicked = st.button(
                "Stop",
                key="btn_stop_mfi_screener",
                **STRETCH,
                **icon_kwargs("stop_circle"),
            )

        if stop_clicked:
            st.session_state["stop_mfi_scan"] = True
        ...
        if run_clicked:
            ...
            if total_tickers == 0:
                ...
                pbar_mfi = st.progress(0)
                pstatus_mfi = st.empty()

                def _progress(done, total):
                    pbar_mfi.progress(min(1.0, done / total) if total else 0)
                    pstatus_mfi.text(f"Scanning MFI: {done}/{total} tickers...")
                ...
                df_mfi_all, info = run_mfi_screener(
                    all_tickers,
                    progress_callback=_progress,
                    phase_callback=_phase,
                    data=data_map,
                    should_stop=lambda: st.session_state.get("stop_mfi_scan", False),
                )
                if st.session_state.get("stop_mfi_scan", False):
                    pstatus_mfi.warning("Screening process cancelled.")
                else:
                    pstatus_mfi.empty()
                pbar_mfi.empty()
                ...
                df_mfi_bullish = pd.DataFrame()
                df_mfi_bearish = pd.DataFrame()
                if not df_mfi_all.empty and "Pattern" in df_mfi_all.columns:
                    df_mfi_bullish = df_mfi_all[
                        df_mfi_all["Pattern"].str.contains("Bullish", case=False, na=False)
                    ]
                    df_mfi_bearish = df_mfi_all[
                        df_mfi_all["Pattern"].str.contains("Bearish", case=False, na=False)
                    ]

                st.session_state["mfi_stats"] = {
                    "total": total_tickers,
                    "success": info["success"],
                    "failed": info["failed"],
                    "bullish_count": len(df_mfi_bullish),
                    "bearish_count": len(df_mfi_bearish),
                    "matched": len(df_mfi_all),
                }
                st.session_state["mfi_source_text"] = source_text
                st.session_state["df_mfi_bullish"] = df_mfi_bullish
                st.session_state["df_mfi_bearish"] = df_mfi_bearish

                # Pilihan awal: saham teratas; kalau hasil kosong, pilihan lama dikosongkan
                if not df_mfi_bullish.empty:
                    first = df_mfi_bullish.iloc[0]
                    st.session_state["selected_mfi_ticker"] = first.get("Ticker", first.get("Saham"))
                    st.session_state["active_mfi_type"] = "Bullish"
                elif not df_mfi_bearish.empty:
                    first = df_mfi_bearish.iloc[0]
                    st.session_state["selected_mfi_ticker"] = first.get("Ticker", first.get("Saham"))
                    st.session_state["active_mfi_type"] = "Bearish"
                else:
                    st.session_state["selected_mfi_ticker"] = None
                st.session_state["mfi_screener_mode_select"] = st.session_state["active_mfi_type"]

        st.markdown("<div style='margin-bottom: 4px;'></div>", unsafe_allow_html=True)

        has_results = "mfi_stats" in st.session_state
        ...
        if has_results:
            ...
            with col_filter:
                screener_mode = st.selectbox(
                    ...
                    **({} if "mfi_screener_mode_select" in st.session_state else {"index": 0}),
                    format_func=lambda v: _MODE_LABELS.get(v, v),
                    key="mfi_screener_mode_select",
                    label_visibility="collapsed",
                )

            with col_export:
                df_export = (
                    st.session_state.get("df_mfi_bullish", pd.DataFrame())
                    if screener_mode == "Bullish"
                    else st.session_state.get("df_mfi_bearish", pd.DataFrame())
                )
            ...
            st.session_state["active_mfi_type"] = screener_mode

            st.markdown(
                source_note_html(st.session_state.get("mfi_source_text", "")),
                unsafe_allow_html=True,
            )
            ...
            if not df_target.empty:
                with st.container(height=800, border=False):
                    for idx, row in df_target.iterrows():
                        ticker = str(row.get("Ticker", row.get("Saham", "")))
                        saham = row.get("Saham", ticker.replace(".JK", ""))
                        is_selected = st.session_state.get("selected_mfi_ticker") == ticker

                        with st.container():
                            st.markdown(_mfi_card_html(row, is_selected), unsafe_allow_html=True)
                            ...
                            if st.button(
                                btn_label,
                                key=f"select_mfi_btn_{ticker}_{idx}",
                                **STRETCH,
                                type=btn_type,
                                **(icon_kwargs("check") if is_selected else {}),
                            ):
                                st.session_state["selected_mfi_ticker"] = ticker
                                st.rerun()
    ...
    # =========================================================
    with col_right:
        if "mfi_stats" in st.session_state:
            stats = st.session_state["mfi_stats"]
        ...
        selected_mfi_symbol = st.session_state.get("selected_mfi_ticker")

        if selected_mfi_symbol:
            if not selected_mfi_symbol.endswith(".JK") and "." not in selected_mfi_symbol:
                selected_mfi_symbol += ".JK"
            ...
            st.markdown(
                ...
                <div style="background: linear-gradient(135deg, rgba(0, 243, 255, 0.12) 0%, rgba(255, 0, 127, 0.1) 100%); border: 1.5px solid #00F3FF; padding: 8px 14px; border-radius: 8px; color: #FFFFFF; font-weight: 600; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 0 12px rgba(0, 243, 255, 0.25);">
                    <span>{svg_icon("target", 18, "#00F3FF", 2, margin_right=6)}SELECTED SYMBOL: <strong style="color: #00F3FF; font-size: 15px; margin-left: 6px;">{selected_mfi_symbol}</strong></span>
            ...
            try:
                render_inline_trade_planner(selected_mfi_symbol, key_suffix="mfi_tab")
            except KeyError as ke:
                if "Status Candle" in str(ke):
                    st.warning(f"Trade Plan rendered with partial data for {selected_mfi_symbol}.")
                else:
                    st.error(f"Failed to load Trade Plan for {selected_mfi_symbol}: {ke}")
            except Exception as e:
                st.error(f"Failed to load Trade Plan for {selected_mfi_symbol}: {e}")
        else:
            st.markdown(
                _help_html(
                    """
                <div class="empty-card" style="text-align: left; padding: 20px; background-color: #161B22; border-radius: 8px; border: 1px solid #30363D;">
                    ...
                    <p style="font-size: 13px; color: #C9D1D9; margin-bottom: 12px; line-height: 1.5;">
                        Screener menemukan divergence MFI yang sudah dikonfirmasi MFI GC/DC (maks. H+3 dari titik ke-2). 
                    ...
                    <ul style="font-size: 12px; color: #8B949E; margin-left: 0; padding-left: 20px; line-height: 1.6; margin-bottom: 16px;">
                        <li style="margin-bottom: 6px;">
                            <strong style="color: #C9D1D9;">Garis divergence:</strong> Tarik garis titik 1 ke titik 2 di harga dan di MFI, pastikan polanya bersih dan tidak ada candle yang menembusnya.
                        </li>
                        <li style="margin-bottom: 6px;">
                            <strong style="color: #C9D1D9;">Konfirmasi candle:</strong> Lebih kuat kalau harga close di atas high kemarin atau muncul candle pembalikan, bukan cuma MFI yang naik.
                        </li>
                        <li style="margin-bottom: 6px;">
                    ...
                    <p style="font-size: 12px; color: #E3B341; margin-top: 8px; margin-bottom: 0; font-weight: 500;">
                        __I_ALERT__ Pill <b>Volatilitas tinggi</b>: saham ini bergerak sangat liar. Selisih 1-2 poin MFI
