...
def load_daftar_saham(filename=os.path.join("data", "daftar_saham.txt")):
    """Reads ticker list (kode tanpa .JK) memakai pembaca yang sama dengan screener dan tugas harian."""
    try:
        target_path = filename if os.path.exists(filename) else find_ticker_file(os.path.basename(filename))
        if not target_path:
            return []
        return [t[:-3] for t in read_ticker_file(target_path)]
    except Exception:
        return []
...
def process_single_ticker(ticker_code: str, df_hist=None):
    ...
    try:
        ...
        planner.fetch_and_prepare_data(data=df_hist)
        ...
        for idx, p in df_plan.iterrows():
            ...
            processed_plans.append({
                ...
                "TP1 Source": str(p.get("TP1 Source", "")),
                "TP2 Source": str(p.get("TP2 Source", "")),
            })
...
def _hist_for(ticker, data_map):
    """Data harian dari file bersama untuk satu ticker (None = ambil langsung)."""
    if not data_map:
        return None
    return data_map.get(normalize_ticker(ticker))


def run_batch_execution(ticker_list, cache_key, data_map=None, source_text=""):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        future_to_ticker = {
            executor.submit(process_single_ticker, t, _hist_for(t, data_map)): t
    ...
    if source_text:
        n_live = sum(1 for t in ticker_list if data_map and _hist_for(t, data_map) is None)
        extra_txt = f" · {n_live} saham tidak ada di file, diambil langsung" if n_live else ""
        st.session_state[f"{cache_key}_source"] = source_text + extra_txt
    ...
    if not all_results:
        ...
        st.warning(f"Tidak ada data valid untuk ticker: {shown}")
...
def render_trade_plan_cards(df_data, is_title_needed=True, is_single_mode=False):
    ...
    for sym in symbols:
        ...
        tp1_src_val = str(row.get("TP1 Source", "") or "")
        tp2_src_val = str(row.get("TP2 Source", "") or "")
        tp1_src_txt = f" Sumber: {html_escape(tp1_src_val)}." if tp1_src_val else ""
        tp2_src_txt = f" Sumber: {html_escape(tp2_src_val)}." if tp2_src_val else ""
        as_of_color = "#64748b" if bool(row.get("Candle Final", True)) else "#fbbf24"
        ...
        if as_of_text:
            copy_extra += f"\n{as_of_text}"
        if tp1_src_val or tp2_src_val:
            copy_extra += f"\nSumber Target: TP1 {tp1_src_val or '-'}, TP2 {tp2_src_val or '-'}"

        copyable_text = f"""=== TRADE PLAN: {sym} ===
Strategy: {strat_code_val}
Grade: {strip_emoji(grade_val)}
Score: {score_val}/100
...
==============================="""
        ...
        with col_c:
            ...
            <button id="{unique_btn_id}" style="width: 100%; background: linear-gradient(135deg, #A855F7 0%, #00F0FF 100%); color: #050811; border: none; padding: 7px 8px; border-radius: 6px; font-weight: 800; font-size: 11px; cursor: pointer; box-shadow: 0 0 8px rgba(0, 240, 255, 0.4); transition: all 0.2s;">
                {svg_icon("copy", 13, "#050811", 2, margin_right=4)}Copy
            </button>
            <script>
        ...
        as_of_html = (
            f'<div style="font-size: 0.72rem; color: {as_of_color}; margin-top: 2px;">{html_escape(as_of_text)}</div>'
            if as_of_text
            else ""
        )
        ...
        with col1:
            ...
            draw_card(
                title="TARGET 1 (TP 1)",
                value=f"Rp {tp1_val:,}",
                subtext=f"Initial profit target / partial exit.{tp1_src_txt}",
        ...
        with col2:
            ...
            draw_card(
                title="TARGET 2 (TP 2)",
                value=f"Rp {tp2_val:,}",
                subtext=f"Main swing target zone.{tp2_src_txt}",
        ...
        st.markdown(
            ...
            <div style="background: #0f172a; border: 1px solid #1e293b; border-radius: 6px; padding: 14px 18px; margin-bottom: 28px; display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px;">
                ...
                <div style="grid-column: span 2;">
                    ...
                    <span style="font-size: 0.88rem; color: {warning_color}; font-weight: 600;">{emoji_to_icons(warning_val)}</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    if st.session_state["screener_mode"] == "single":
        ...
        if btn_single or enter_pressed:
            if not input_ticker.strip():
                st.warning("Input ticker list is empty.")
            else:
                list_to_scan = [
                    t.strip().upper()
        ...
        if active_cache_key in st.session_state:
            ...
            if df_single_res.empty:
                st.warning("No valid data returned for the selected tickers.")
            else:
        ...
        if active_cache_key in st.session_state:
            ...
            st.write("")
            with st.expander("**Filters & Criteria**", expanded=True, **expander_kwargs("tune")):
            ...
            source_note = st.session_state.get("df_screener_batch_source", "")
            if source_note:
                st.markdown(
                    f'<div style="font-size: 0.75rem; color: #64748b; margin: 2px 0 8px 0;">{html_escape(source_note)}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if not selected_rows.empty:
                ...
                for _, r in df_to_preview.iterrows():
                    ...
                    if as_of_r:
                        extra += f"\n{as_of_r}"
                    if str(r.get("TP1 Source", "") or "") or str(r.get("TP2 Source", "") or ""):
                        extra += f"\nSumber Target: TP1 {r.get('TP1 Source', '') or '-'}, TP2 {r.get('TP2 Source', '') or '-'}"
                    p_text = f"""=== TRADE PLAN: {r.get('Symbol', '')} ===
Strategy: {r.get('Strategy', 'BOW')}
Grade: {strip_emoji(r.get('Grade', 'N/A'))}
...
==============================="""
            ...
            with c_act_copy:
                ...
                <button id="{unique_batch_btn_id}" style="width: 100%; background: linear-gradient(135deg, #A855F7 0%, #00F0FF 100%); color: #050811; border: none; padding: 9px 10px; border-radius: 6px; font-weight: 800; font-size: 13px; cursor: pointer; box-shadow: 0 0 8px rgba(0, 240, 255, 0.4); transition: all 0.2s;">
                    {svg_icon("copy", 13, "#050811", 2, margin_right=4)}Copy Selected
                </button>
                <script>
                ...
                const orig_{unique_batch_btn_id} = btn_{unique_batch_btn_id}.innerHTML;
                btn_{unique_batch_btn_id}.onclick = function() {{
                    ...
                    navigator.clipboard.writeText(textToCopy_{unique_batch_btn_id}).then(function() {{
                        btn_{unique_batch_btn_id}.textContent = "Copied";
                        btn_{unique_batch_btn_id}.style.background = "#00FF66";
                        setTimeout(function() {{
                            btn_{unique_batch_btn_id}.innerHTML = orig_{unique_batch_btn_id};
            ...
            with c_act_wl:
                if st.button("Watchlist Selected", **STRETCH, key="btn_wl_batch_selected", **icon_kwargs("bookmark_add")):
                    if not selected_rows.empty:
                        ...
                        st.toast("Pilih minimal satu saham dicentang pada tabel di atas.")
