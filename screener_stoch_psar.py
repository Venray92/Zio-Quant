...
def run_stoch_psar_screener(tickers=None):
    """Jalankan screening Stochastic + Parabolic SAR.

    Returns: df_gc (Golden Cross / Buy), df_dc (Dead Cross / Sell)
    """
    if tickers is None:
    ...
    for ticker in tickers:
        try:
            df = yf.download(
                ticker, period="90d", interval="1d", progress=False
            )
            ...
            fast_k = 100 * (
                (df["Close"] - low_min10) / (high_max10 - low_min10)
            )
            ...
            if k0 < 35:
                ...
                gc_2days_ago = (
                    (k3 < d3) and (k2 >= d2) and (k1 >= d1) and (k0 >= d0)
                )
                gc_3days_ago = (
                    (k4 < d4)
                    and (k3 >= d3)
