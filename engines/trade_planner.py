...
class TradePlanner:
    ...
    def get_direction(self):
        ...
        latest_high_val = self.highs_5.sort_values(by="Date", ascending=False).iloc[0]["High"]
        latest_low_val = self.lows_15.head(5).sort_values(by="Date", ascending=False).iloc[0]["Low"]
    ...
    def classify_candle(self):
        ...
        if p1["is_green"] and p2["is_red"] and (p1["body_top"] >= p2["body_top"]) and (p1["body_bottom"] <= p2["body_bottom"]) and is_large_body:
            return ("Bullish Engulfing (Reversal)" if is_downtrend else "Bullish Engulfing"), "BULLISH"
        ...
        if is_marubozu_body and has_minimal_shadows:
            return ("Bullish Marubozu (Strong)" if p1["is_green"] else "Bearish Marubozu (Strong)"), ("BULLISH" if p1["is_green"] else "BEARISH")
        ...
        if is_hammer_shape:
            return ("Hammer (Valid Reversal)", "BULLISH") if is_downtrend else ("Hanging Man", "BEARISH")

        if is_shooting_shape:
            return ("Shooting Star (Valid Reversal)", "BEARISH") if is_uptrend else ("Inverted Hammer", "NEUTRAL")

        color_str = "Hijau" if p1["is_green"] else "Merah"
        if is_small_body:
            return f"Small {color_str} / Spinning Top", "NEUTRAL"

        return f"Standard {color_str}", ("BULLISH" if p1["is_green"] else "BEARISH")
    ...
    def calculate_score_and_warnings(self, plan_type, buy_min, buy_max, target_1, stop_loss, rr_ratio, candle_type, candle_bias):
        ...
        elif last_close > buy_max and ((last_close - buy_max) / buy_max * 100) <= 2.0:
            score_zone = 15
        ...
        if candle_bias == "BULLISH":
            score_candle = 20 if any(k in candle_type for k in ["Engulfing", "Morning Star", "Soldiers", "Marubozu"]) else 15
        ...
        if plan_type == "BOW":
            dist_to_support = ((last_close - buy_min) / buy_min) * 100 if buy_min > 0 else 0
            if dist_to_support > 4.0:
                warnings.append(f"⚠️ Jauh dari Support (+{round(dist_to_support, 1)}%)")
                penalty += 10

        dist_to_target = ((target_1 - last_close) / last_close) * 100 if last_close > 0 else 0
        if 0 < dist_to_target <= 1.5:
            warnings.append(f"⚠️ Dekat Resistance (Sisa +{round(dist_to_target, 1)}%)")
            penalty += 5
        ...
        total_score = int(score_rr + score_zone + score_candle + score_safety)
        ...
        warning_str = " | ".join(warnings) if warnings else "✅ Setup Clean / Minimum Risk"
    ...
    def generate_trade_plan(self):
        ...
        def find_target_1(min_val):
            if not self.strong_resistance.empty:
                valid_res = sorted([p for p in self.strong_resistance["High"].values if (p - min_val) >= min_point_gap])
                if valid_res:
        ...
        def find_target_2(target_1):
            ...
            if not self.strong_resistance.empty:
                valid_res = sorted([p for p in self.strong_resistance["High"].values if p >= min_tp2])
                if valid_res:
                    return self.round_to_nearest_tick(valid_res[0])
        ...
        score_bow, grade_bow, pos_bow, warn_bow = self.calculate_score_and_warnings(
            "BOW", rb_bow_min, rb_bow_max, target_1_bow, stop_loss_bow, rr_val_bow, candle_name, candle_bias
        )

        # 2. BOB PLAN
        if not self.strong_resistance.empty:
            res_sorted = self.strong_resistance.sort_values(by="Date", ascending=False)
        ...
        score_bob, grade_bob, pos_bob, warn_bob = self.calculate_score_and_warnings(
            "BOB", base_bob_high, upper_bob_high, target_1_bob, stop_loss_bob, rr_val_bob, candle_name, candle_bias
        )

        def fmt_range(p_min, p_max):
            p_min_str = f"{int(p_min):,}" if p_min.is_integer() else f"{p_min:,}"
            p_max_str = f"{int(p_max):,}" if p_max.is_integer() else f"{p_max:,}"
