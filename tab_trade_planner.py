...
def _fmt_as_of(value, final=True):
    ...
    try:
        ...
        return ""
    return f"Data per {txt}" + ("" if final else " · ⚠️ candle belum final")
...
def _load_market_file_cached(repo):
    return load_market_file(repo)


def prepare_batch_source():
    """Tentukan sumber data batch. Return (data_map atau None, teks sumber)."""
    now = now_wib()
    meta, df = None, None
    if not is_live_window(now):
        ...
        if repo:
            try:
                df, meta = _load_market_file_cached(repo)
            except Exception:
                df, meta = None, None
    ...
    if not calendar_is_covered(now.date()):
        note = f" · ⚠️ kalender libur bursa {now.year} belum diisi"
    if source == "file":
    ...
    return None, f"Sumber data: live (Yahoo) · diambil {stamp} WIB · {reason}{note}"
