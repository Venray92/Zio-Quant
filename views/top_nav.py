...
def render_top_nav():
    ...
    st.markdown(
        ...
        /* Mengatur layout teks & icon di dalam button */
        button[key*="nav_"] p {
        ...
        /* Ukuran ikon SVG kecil presisi (16px x 16px) */
        .nav-btn-icon {
    ...
    # 1. HOME
    with cols[0]:
        btn_label = f"{make_img_tag(SVG_HOME_RAW)} Home"
        if st.button(btn_label, key="nav_home", use_container_width=True):
    ...
    # 2. WATCHLIST
    with cols[1]:
        btn_label = f"{make_img_tag(SVG_WATCHLIST_RAW)} Watchlist"
        if st.button(btn_label, key="nav_watchlist", use_container_width=True):
            st.session_state["selected_page"] = "watchlist"
            st.rerun()

    # 3. MONEY MANAGEMENT
    with cols[2]:
        btn_label = f"{make_img_tag(SVG_MM_RAW)} Money Management"
        if st.button(btn_label, key="nav_mm", use_container_width=True):
            st.session_state["selected_page"] = "money_management"
            st.rerun()

    # 4. HOW TO
    with cols[3]:
        btn_label = f"{make_img_tag(SVG_HOWTO_RAW)} How To"
        if st.button(btn_label, key="nav_howto", use_container_width=True):
